...
export default function RoomScheduler() {
  ...
  const getRoomName = (value: ResourceDetails) => {
    return value?.resourceData?.[value?.resource?.textField as keyof typeof value.resourceData] ?? 'Unknown';
  }
  ...
  const onActionBegin = (args: ActionEventArgs): void => {
    if (args.requestType === 'eventCreate' || args.requestType === 'eventChange') {
      ...
      if (scheduleObj.current && !scheduleObj.current.isSlotAvailable(data)) {
        args.cancel = true;
      }
    }
  }
  ...
  //         (scheduleObj.current && !scheduleObj.current.isSlotAvailable(data))) {
  //         args.cancel = true;
  //       }
  ...
  const onPopupOpen = (args: PopupOpenEventArgs): void => {
    ...
    if (args.type === 'QuickInfo' || args.type === 'Editor' || args.type === 'RecurrenceAlert' || args.type === 'DeleteAlert') {
      let target: HTMLElement = (args.type === 'RecurrenceAlert' ||
        args.type === 'DeleteAlert') ? args.element as HTMLElement : args.target as HTMLElement;
      if (!isNullOrUndefined(target) && target.classList.contains('e-work-cells')) {
        if ((target.classList.contains('e-read-only-cells')) ||
          (scheduleObj.current && !scheduleObj.current.isSlotAvailable(data))) {
          args.cancel = true;
